...
CACHE=ROOT/"cache/stage06b2_sentinel_fold3"
OUT=ROOT/"reports/stage06b2_sentinel_fold3"

FROZEN_ALPHA=.20
DEPTH=30
MAXLEN=1024
OVERLAP=.50
SENTINEL_FOLD="fold_3"

SCORES=CACHE/"fold3_scores.f32.npy"
DONE=CACHE/"fold3_done.u1.npy"
META=CACHE/"fold3_scores.json"
...
def verify(qids,idx):
    ...
    contract={
        "schema":"stage06b2.sentinel.fold3.v1",
...
def prep(idx,shortlist,docs):
    ...
    vv,rr,sim,views=a0.select_witnesses("bge_lora_sentinel_fold3_top30",idx,local,docs)
...
def main():
    ...
    print("[1/5] load fold3 sentinel world",flush=True)
    ...
    if delta<=0:dec="KILL_BGE"
    elif delta<.0015:dec="ARCHIVE_WEAK_COMPLEMENT"
    else:dec="PASS_TO_FOLD4_CONFIRMATION"
    ...
    rep={"schema":"dsc2026.endgame.stage06b2.sentinel.fold3.v1","status":"COMPLETE",
         ...
         "decision":dec}
    (OUT/"SENTINEL.json").write_text(json.dumps(rep,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    (OUT/"REPORT.md").write_text(
        "# Stage06B2 Sentinel Fold3\n\n"
    ...
    print("="*105)
    print("FOLD3 SENTINEL — alpha(BGE)=0.20 FROZEN")
